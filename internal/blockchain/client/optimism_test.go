...
func (s *optimismClientTestSuite) TestOptimismTest_GetBlockByHeight_WithBestEffort() {
	...
	block, err := s.client.GetBlockByHeight(context.Background(), tag, 24000000, WithBestEffort())
	require.NoError(err)
...
// traces for them.
func (s *optimismClientTestSuite) TestOptimismTest_GetDuplicateTransaction() {
	require := testutil.Require(s.T())
	...
	// Check fake receipt
	var fullReceipt parser.EthereumTransactionReceipt
