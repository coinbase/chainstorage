...
func (s *arbitrumIntegrationTestSuite) TestArbitrumGetBlock_AfterNitroUpgrade_GethNestedTraces() {
	const (
		arbitrumTag        = uint32(1)
		arbitrumBlockHash  = "0xd1882c626699cd19548720713669993ac8f51500056dfbc1afc180496c7f8e2f"
		arbitrumHeight     = uint64(22207818)
		arbitrumParentHash = "0x7d237dd685b96381544e223f8906e35645d63b89c19983f2246db48568c07986"
	)
	...
	for _, test := range tests {
		s.Run(test.name, func() {
			...
			transactionFlattenedTraces := txn.FlattenedTraces
			require.Equal(3, len(transactionFlattenedTraces))
			s.app.Logger().Info("arbitrum txn flattened traces:", zap.Reflect("transaction_flattened_traces", transactionFlattenedTraces))

			traceFrom := "0x00000000000000000000000000000000000a4b05"
			traceTo := "0x00000000000000000000000000000000000a4b05"
			require.Equal("CALL", transactionFlattenedTraces[0].Type)
			require.Equal(traceFrom, transactionFlattenedTraces[0].From)
			require.Equal(traceTo, transactionFlattenedTraces[0].To)
			require.Equal("0", transactionFlattenedTraces[0].Value)
			require.Equal(uint64(2), transactionFlattenedTraces[0].Subtraces)
			require.Equal([]uint64{}, transactionFlattenedTraces[0].TraceAddress)
			require.Equal(arbitrumHeight, transactionFlattenedTraces[0].BlockNumber)
			require.Equal(arbitrumBlockHash, transactionFlattenedTraces[0].BlockHash)
			require.Equal(transactionHash, transactionFlattenedTraces[0].TransactionHash)
			...
			require.Equal("CALL", transactionFlattenedTraces[0].CallType)
			require.Equal("CALL", transactionFlattenedTraces[0].TraceType)
			require.Equal("CALL_0xcb91013e1bf7d0131ac1cfee691dfd6bc833e6285126976a74e352767f72e954", transactionFlattenedTraces[0].TraceId)
			require.Equal(uint64(1), transactionFlattenedTraces[0].Status)
		})
	}
}
