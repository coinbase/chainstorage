...
func (s *ethereumRosettaParserTestSuite) TestEthereumRosettaParser() {
	...
	secondTransactionMetadata, _ := rosetta.FromSDKMetadata(map[string]any{
		"gas_limit": "0x15f90",
...
func (s *ethereumRosettaParserTestSuite) TestParseBlock_WithdrawAmount() {
	require := testutil.Require(s.T())

	targetTxHash := "WITHDRAWAL_0x79ffaa2840f06eee8f41c61670b132382a988612b25b9a7deca0bea0691db491_292939"
	block, err := testutil.LoadRawBlock("parser/ethereum/raw_block_17053184.json")
	require.NoError(err)
	rosettaBlock, err := s.parser.ParseRosettaBlock(context.Background(), block)
	require.NoError(err)
	var targetTx *rosetta.Transaction
	for _, txn := range rosettaBlock.Block.Transactions {
		if txn.TransactionIdentifier.Hash == targetTxHash {
			targetTx = txn
		}
	}
	op := targetTx.Operations[0]
	require.Equal(op.Amount.Value, "36541948083000000000")
}
