...
const (
	blockAuthor = `"0xf0245f6251bef9447a08766b9da2b07b28ad80b0"`
)

type polygonParserTestSuite struct {
	suite.Suite
...
func TestPolygonParserTestSuite(t *testing.T) {
	suite.Run(t, new(polygonParserTestSuite))
}

func (s *polygonParserTestSuite) SetupTest() {
...
func (s *polygonParserTestSuite) TearDownTest() {
	s.testapp.Close()
	s.ctrl.Finish()
}
	...
	block := &api.Block{
		...
		Blobdata: &api.Block_Ethereum{
			Ethereum: &api.EthereumBlobdata{
				Header:              fixtureHeader,
				TransactionReceipts: [][]byte{fixtureReceipt},
				TransactionTraces:   [][]byte{fixtureTransactionTraceWithGasAsNumber},
			},
		},
	}
...
func (s *polygonParserTestSuite) TestParsePolygonBlock_WithAuthor() {
	require := testutil.Require(s.T())
	...
	actual := nativeBlock.GetEthereum()
	require.NotNil(actual)
	require.Equal("0x0000000000000000000000000000000000000000", actual.Header.Miner)
	require.Equal("0xf0245f6251bef9447a08766b9da2b07b28ad80b0", actual.Header.GetAuthor())
}

func (s *polygonParserTestSuite) TestParsePolygonBlock_CompatibleWithoutAuthor() {
	require := testutil.Require(s.T())
	...
	require.NotNil(actual)
	require.Equal("0x0000000000000000000000000000000000000000", actual.Header.Miner)
	require.Empty(actual.Header.GetAuthor())
}
